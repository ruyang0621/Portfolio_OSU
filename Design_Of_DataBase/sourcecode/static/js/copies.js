...
for(let i=0; i< delBtns.length; i++)
{
    delBtns[i].addEventListener('click', function(){
        postData('copies', { id: this.getAttribute('data-id'), action: 'delete_copy' }, 'DELETE')
        .then(data => {
            console.log(data)
            if(data['result'] == 'fail') {
                alert('Oops! The copy has relation with a member, please delete the relation at BorrowRecords first.');
            } else {
                alert('The copy has been deleted.');
            }
            location.reload();
        });
    })
...
const IdInput = document.getElementById('copyId');
...
for(let i=0; i< editBtns.length; i++)
{
    editBtns[i].addEventListener('click', function(){
        ...
        let book_status = this.getAttribute('data-book-status');
        let book_condition = this.getAttribute('data-book-condition');

        conditionInput.value = book_condition;
        statusInput.value = book_status;
        IdInput.value = Id;
